...
## 7. 阶段收束与下一阶段入口（→ README_13）

本篇收束时的固件基线：

```text
3084e4f 回退: lepton.c 采集路径整体恢复 2FPS 金标准 (HEX MD5 C87FBCC8)
```

阶段结论：

1. **采集路径**：恢复 2FPS 金标准（persistent shelf 语义），fresh 门禁三次尝试全部
   回退（§5.2/§5.5/§5.6），"运动拼接帧"防线放在 Qt 撕裂门禁（§4）；
2. **FPS 优化暂停**：采集时序优化（README_9 §7 Step3/Step4 路线）挂起，避免在
   无完整诊断依据下继续理论修补；
3. **下一阶段转向 ESP32 无线链路**：STM32(SPI5 从机)→ESP32-S3→WiFi TCP→Qt
   WiFi热成像模式，含 UART4/TCP 双链路运行时切换、PG2 就绪线、PJ7 烧录时序。
   设计、落地与评审记录见 **README_13**。AA55 帧协议保持不变，Qt/Android 解码
   零改动（AGENTS.md 协议兼容性要求）。
