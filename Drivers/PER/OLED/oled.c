...
/**
  * @brief  在指定位置显示一个字符 (支持 0806/1206/1608/2412 统一逐列式字库)
  * @param  x, y: 字符显示的左上角起点坐标
  * @param  chr: 待显示的字符 (ASCII)
  * @param  size1: 选择字号高度 (8 / 12 / 16 / 24)
...
void OLED_ShowChar(uint8_t x, uint8_t y, uint8_t chr, uint8_t size1, uint8_t mode)
{
    ...
    // 1. 根据高度，自动计算该字号的“宽度(w)”与“所占Page页数(p)”
    if (h == 12)
    {
        w = 6;
        p = 2;
        font_ptr = oled_asc2_1206[chr1];
    }
    else if (h == 16)
    {
        w = 8;
        p = 2;
        font_ptr = oled_asc2_1608[chr1];
    }
    ...
    for (i = 0; i < w; i++) // 遍历所有列
    {
        for (page = 0; page < p; page++) // 遍历当前列的所有页
        {
            // 恢复正常的物理页顺序
            temp = font_ptr[i * p + page]; 

            for (m = 0; m < 8; m++)
            {
                // 核心改变：temp & 0x80 配合 temp <<= 1 实现了“高位在前”的像素输出
                if (temp & 0x80)
                {
                    OLED_DrawPoint(x0 + i, y0 + page * 8 + m, mode);
                }
                else
                {
                    OLED_DrawPoint(x0 + i, y0 + page * 8 + m, !mode);
